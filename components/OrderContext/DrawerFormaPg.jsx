import { Button, Drawer, DrawerBody, DrawerContent, DrawerHeader, DrawerOverlay, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Radio, RadioGroup, Stack, Text, useDisclosure, useToast } from '@chakra-ui/react'
...
function DrawerFormaPg({ setOpenFormaPg, openFormaPg, formasPg, subdomain, getAllFormasPg, setOrder, order, total }) {
    ...
    return (
        <>
            <Drawer placement='bottom' onClose={() => setOpenFormaPg(false)} isOpen={openFormaPg}>
                <DrawerOverlay />
                <DrawerContent>
                        ...
                        <RadioGroup mt='12px' value={parseInt(forma)} onChange={setForma}>
                            <Stack>
                                {formasPgData.map((item, index) => (
                                    <Radio key={index} size='md' value={parseInt(item.id)} colorScheme='blue'>
                                        {item.descricao}
                                    </Radio>
                                ))}
                            </Stack>
                ...
                <ModalContent>
                    ...
                    <ModalBody bg='rgb(243, 245, 247)'>
                        <Formik
                            ...
                            initialValues={{}}
                            onSubmit={(values) => {
                                let valueFinal = values.troco.replaceAll('R$ ', '');

                                if (formaSelectedData?.requer_troco && valueFinal !== undefined) {
                    ...
                    <ModalFooter>
                        ...
                        <Button form='formTroco' type='submit' variant='btnDallas' color='white'>Salvar</Button>
                    </ModalFooter>
                </ModalContent>
            </Modal>
        </>
