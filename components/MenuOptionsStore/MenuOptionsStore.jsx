...
function MenuOptionsStore({ data, categories, products, getAll, subdomain }) {
    ...
    return (
        <Box id='ref' w='100%' bg={data?.primary_color} h='75px' display='flex' padding={['12px 30px', '12px 50px']} alignItems='center' justifyContent='center' css={
            `
                &.minimized{
                    position: fixed;
                    top: 88px;
            ...
            {!isLoading && (
                <Box ref={internalSliderRef} className="keen-slider" alignItems='center'>
                    {categoriesData.map((item, key) => productsData?.filter((entry) => filterGpProduct(entry, item)).length > 0 && (
                        <Box className="keen-slider__slide" key={key}>
                            <Box _hover={{
                                opacity: 0.8
                            }} textAlign='center' cursor='pointer' borderTop={active == item.id_grupo ? '2px solid' : '0px'} pt='8px' pb='8px' borderColor='#fff'>
                                <AnchorLink offset='300' href={'#' + slugify(item.descricao, { lower: true })} onClick={() => setActive(item.id_grupo)}>
                                    <Text color='white' fontSize={['12px', '15px']} lineHeight={['14px', 'auto']} fontWeight={600}>
                                        {item.descricao}
                                    </Text>
                                </AnchorLink>
                            </Box>
